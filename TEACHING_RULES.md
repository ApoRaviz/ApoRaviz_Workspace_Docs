...
### Accessible Technical Vocabulary Rule

คำว่า "เข้าใจง่าย" ไม่ได้แปลว่าตัดศัพท์ technical ออกทั้งหมด เพราะ learner ต้องนำศัพท์ไปอ่าน documentation, อ่าน error และสื่อสารกับ Programmer คนอื่นได้ในอนาคต

ให้สอนศัพท์ตามลำดับ:

```text
ความหมายแบบคนธรรมดา
-> ชื่อ technical ที่ใช้จริง
-> ชี้ว่าตรงกับส่วนใดใน code หรือ output
-> ใช้ศัพท์เดิมซ้ำอย่างสม่ำเสมอ
```

ตัวอย่างครั้งแรกที่เจอ:

```text
คำสั่งตรวจว่าผลตรงตามที่คาดหรือไม่
เรียกว่า assertion
ใน code นี้คือ expect(...)
```

หลังเริ่มคุ้นจึงย่อเป็น:

```text
assertion หรือ expect()
```

ห้ามใช้ศัพท์ technical หนึ่งคำไปอธิบายด้วยศัพท์ technical ใหม่หลายคำโดยไม่แปล และห้ามทำให้ Knowledge Check ยากขึ้นเพียงเพราะภาษาของตัวเลือกยากกว่าบทเรียน

เมื่อ flow มีผู้ทำงานหลายตัว เช่น Service, HttpClient, testing backend และ test runner ให้เรียงคำอธิบายดังนี้:

```text
แยกของจริงกับของจำลอง
-> ทำแผนที่ว่าใครมีหน้าที่อะไร
-> ตามข้อมูลว่าใครส่งอะไรให้ใคร
-> ผูกชื่อ technical กลับไปยัง code
-> ระบุว่าสิ่งนี้พิสูจน์อะไรและไม่พิสูจน์อะไร
```

No Black Box หมายถึงไม่มีขั้นตอนสำคัญหายไป ไม่ได้หมายความว่าต้องเปิดรายละเอียดภายใน framework ทุกชั้นก่อน learner เห็นภาพรวม
...
ก่อนเริ่มสอนใน turn แรกของแชทใหม่ ให้ AI เช็กวันที่และ timezone จริง แล้วเทียบกับวันที่ของ teaching checkpoint ล่าสุดที่เชื่อถือได้ ถ้าเป็นแชทใหม่, วันที่ไม่ตรงกัน หรือหา checkpoint date ไม่ได้ ให้ถือว่า context อาจขาดและทำรายการนี้ก่อน:

1. อ่านกฎ project และ `TEACHING_RULES.md` ฉบับเต็ม
2. ตรวจ working tree, diff, progress checkpoint และสิ่งที่ learner ทำค้างไว้
3. สรุปว่าบทอยู่ตรงไหนและส่วนย่อยถัดไปมีขอบเขตอะไร
4. สำรวจศัพท์, syntax, class, decorator และ API ใน snippet ถัดไป; เรื่องใหม่ต้องปูพื้นก่อนให้ learner วาง ส่วนความรู้เก่าที่ flow ใหม่ต้องใช้ให้ทวนแบบ just-in-time

ห้ามเริ่มสอนเนื้อหาใหม่หรือให้ learner เพิ่ม code ก่อนทำรายการที่เข้าเงื่อนไขนี้ครบ กฎนี้ไม่บังคับให้อ่านเอกสารทั้งหมดซ้ำทุก turn ในวันเดียวกัน และห้ามใช้เป็นเหตุผลขยาย scope ออกจากบทปัจจุบัน
...
- กระจายตำแหน่งคำตอบที่ถูกระหว่าง A-E อย่างสมดุลในชุดคำถามต่อเนื่อง และไม่ใช้ตัวอักษรเดิมเกิน 2 ข้อติดกัน เว้นแต่มีเหตุผลด้านเนื้อหาที่หลีกเลี่ยงไม่ได้
- การกระจายตำแหน่งห้ามสำคัญกว่าความถูกต้อง: ทุกข้อยังต้องมีคำตอบดีที่สุดเพียงข้อเดียวและตัวลวงสมเหตุสมผล

### Retention & Review Rule

เมื่อ learner กังวลว่าจำสิ่งที่เรียนมาไม่ได้ อย่าแนะนำให้อ่านเอกสารทั้งหมดตั้งแต่ต้นเป็นวิธีหลัก ให้ใช้ลำดับนี้:

```text
ลองนึกหรืออธิบายจากความจำก่อน
-> เปิดเฉพาะ topic/File Map ที่นึกไม่ออก
-> ปิดเอกสารแล้วอธิบายใหม่ด้วยคำของตัวเอง
-> ทบทวนซ้ำแบบเว้นระยะ
-> แทรก cumulative Knowledge Check จากบทก่อนเป็นระยะ
```

Workspace Docs ทำหน้าที่เป็น external memory และ reference ไม่ใช่สิ่งที่ learner ต้องจำคำต่อคำ เป้าหมายคือจำภาพรวม, รู้ว่าเรื่องใดเกี่ยวกับอะไร และรู้ว่าจะกลับไปค้นตรงไหน

เมื่อบทใหม่ต้องใช้ concept ที่เคยเรียนเป็นฐาน ให้ทำ just-in-time recap สั้น ๆ ก่อนสอนของใหม่: ทวนภาพจำ, flow และคำสำคัญเฉพาะส่วนที่กำลังจะใช้ แล้วจึงเชื่อมเข้าหัวข้อใหม่ ไม่เปิดบทเดิมเต็มบทซ้ำโดยอัตโนมัติ ตัวอย่างเช่น เมื่อ CORS, authentication, rate limiting หรือ monitoring แตะ request pipeline ให้ทวน `request -> middleware -> next/short-circuit -> response` ก่อน
...
เมื่อต้องอธิบาย code หนึ่งบรรทัดที่มีหลายส่วน ให้ใช้วิธี **แยกชิ้นแล้วประกอบกลับ**:

```text
แสดงบรรทัดเต็ม
-> แยกคำหรือส่วนตามหน้าที่
-> อธิบายแต่ละส่วนพร้อมตัวอย่างสั้น
-> ประกอบกลับเป็นความหมายและ flow ของทั้งบรรทัด
```

อย่าเปลี่ยนทุกคำให้เป็นบทเรียนยาวแยกจากกันโดยอัตโนมัติ เพราะจะทำให้ภาพรวมขาดตอน ให้เปิด mini-lesson แยกเฉพาะเมื่อส่วนนั้นเป็นพื้นฐานสำคัญจริง หรือ learner ยังสับสนหลังคำอธิบายสั้น
